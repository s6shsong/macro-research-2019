...
Pe=0.0025;                  % Energy price(data)(***)(这个需要再查找）
...
% Age-efficiency profile(***)(这个需要进一步讨论如何做）（如果要讨论eh和el要分做两个profiles)
e = load('ef.txt');
plot(e)
...
tolc=le-3;              % Numerical tolerance for energy consumption
...
while q<nq && (abs(K1-K0)>tolk ||abs(E1-E0)>tolc|| abs(N1-N0)>tollab)
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % Last period utility
    cons=(1+r0)*kap-(Pe+tauC)*ene;                    % last period consumption (vector!)(***)（总是觉得这里有点奇怪）
   ...
   %   We need to solve for optimal savings decision at age j=J-1 for each(line129:ik0=1:nk, loop in line) value of current  asset holdings        
            ...
            while ik1<nk  	% assets tomorrow
                ...
                cons=(1+r0)*kap0-kap1-(Pe+tauC)*ene;
                
                if cons<=0
                    util=neg;
                else
                    util=((cons.^gamma*ene.^(1-gamma))^(1-theta1))/(1-theta1);
                end
  ...
  % Instantaneous utility
                cons=(1+r0)*kap0+(1-tau)*w0*e(j)*lab-kap1-(Pe+tauC)*ene;
                
                if cons<=0
                    util=neg;
                else
                    util=(((cons^gamma)*((ene)^(1-gamma)))^(1-theta1))/(1-theta1)-(h^(1+(1/theta2)))/(1+(1/theta2));
                end
    ...
    egen=zeros(J,1);        % energy supply E(j+1) for each generation j
    labgen=zeros(tW,1);     % labor supply l(j) for each generation j

    % Use decision rules to iteratively fill in kgen, egen and labgen
    ...
    for j=1:J               % iterations over cohort
      ...
      iE0 = 1;            % starting energy of j=1
      for j = 1:J         % iterations over cohort
          % energy decision ene(e) to find the optimal one
          iE1 = eneopt(iE0,j);
          egen(j) = ene(iE1);
